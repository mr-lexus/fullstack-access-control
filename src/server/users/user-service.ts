...
function normalizeEmail(value: unknown): string {
  ...
  if (!emailPattern.test(email))
    throw new AppError("INVALID_INPUT", "A valid email is required.");
  return email;
}
...
function assertUniqueEmail(email: string, exceptId?: string): void {
  if (
    [...getStore().users.values()].some(
      (user) => user.email === email && user.id !== exceptId,
    )
  )
    throw new AppError("EMAIL_ALREADY_EXISTS", "That email is already in use.");
}
...
export function listUsers(actor: UserRecord): PublicUser[] {
  ...
  const users = can(actor, CAPABILITIES.VIEW_ALL_USERS)
    ...
    : [...getStore().users.values()].filter(
        (user) => user.managerId === actor.id,
      );
...
export function createUser(
  ...
  if (!isRole(input.role))
    throw new AppError("INVALID_INPUT", "Role is invalid.");
  if (!isUserStatus(input.status))
    throw new AppError("INVALID_INPUT", "Status is invalid.");
  const managerId =
    input.managerId === null || input.managerId === undefined
      ? null
      : requiredText(input.managerId, "Manager ID");
  if (managerId && !getStore().users.has(managerId))
    throw new AppError("INVALID_INPUT", "Manager does not exist.");
  assertUniqueEmail(email);
  const user: UserRecord = {
    id: `user-${getStore().users.size + 1}`,
    fullName,
    email,
    password,
    role: input.role,
...
export function updateUserProfile(
  ...
  if (Object.keys(input).some((key) => key !== "fullName" && key !== "email"))
    throw new AppError(
      "INVALID_INPUT",
      "Only fullName and email may be updated.",
    );
  ...
  const fullName =
    input.fullName === undefined
      ? target.fullName
      : requiredText(input.fullName, "Full name");
  const email =
    input.email === undefined ? target.email : normalizeEmail(input.email);
...
function countActiveIt(): number {
  return [...getStore().users.values()].filter(
    (user) => user.role === ROLES.IT && user.status === "active",
  ).length;
}
...
export function changeUserRole(
  ...
  if (
    ...
    throw new AppError(
      "LAST_ACTIVE_IT",
      "The last active IT account cannot lose its IT role.",
    );
  if (actor.id === target.id && roleValue !== target.role)
    throw new AppError(
...
export function changeUserStatus(
  ...
  if (
    ...
    throw new AppError(
      "LAST_ACTIVE_IT",
      "The last active IT account cannot be deactivated.",
    );
  if (
    actor.id === target.id &&
    target.status === "active" &&
