...
export function LoginForm() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    const payload: { redirectTo?: string; error?: { message: string } } = await response.json();
    if (!response.ok) { setError(payload.error?.message ?? "Login failed."); setBusy(false); return; }
