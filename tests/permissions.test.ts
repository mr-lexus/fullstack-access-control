...
describe("manager object-level permissions", () => {
  it("allows Anna to edit exactly her four direct reports", () => {
    ...
    expect(listUsers(anna).map((user) => user.id)).toEqual([
      "olena",
      "taras",
      "nina",
      "bohdan",
    ...
    for (const id of ["olena", "taras", "nina", "bohdan"]) {
      expect(
        updateUserProfile(anna, id, { fullName: "Updated" }).fullName,
      ).toBe("Updated");
    }
  });
  ...
  it("rejects Dmytro, self, and arbitrary users", () => {
    ...
    for (const id of ["dmytro", "anna", "ivan"]) {
      expect(() =>
        updateUserProfile(anna, id, { fullName: "Nope" }),
  ...
  it("rejects a direct report after IT promotes that user to manager", () => {
    ...
    expect(changeUserRole(ivan, "olena", ROLES.MANAGER).role).toBe(
      ROLES.MANAGER,
    );
  ...
  it("does not let profile updates smuggle protected fields", () => {
    expect(() =>
      updateUserProfile(actor("anna"), "olena", { role: "IT" }),
    ).toThrowError(/Only fullName and email/);
